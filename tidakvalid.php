...
// (start ) Inisialisasi variabel untuk menyimpan jumlah data dengan nilai 'Tidak Valid'
$totalTidakValid = 0;
...
while ($data = mysqli_fetch_array($ambildata)) {
    // Memeriksa apakah nilai 'kalibrasi_ulang' kurang dari tanggal sekarang
    $tanggal_sekarang = new DateTime();
    $tanggal_sekarang_formatted = $tanggal_sekarang->format('Y-m-d');
    if ($data['kalibrasi_ulang'] < $tanggal_sekarang_formatted) {
        // Jika ya, maka status diubah menjadi 'Tidak Valid'
        $data['status'] = 'Tidak Valid';
        // Jika status adalah 'Tidak Valid', maka tambahkan satu ke totalTidakValid
        if ($data['status'] === 'Tidak Valid') {
            $totalTidakValid++;
        }
    }
}
        ...
        <div class="container py-5 pt-lg-0">
            <div class="row gx-0">
                ...
                <div class="bg-primary primary d-flex align-items-center justify-content-center p-4" style="height: 150px;">
                        ...
                        <div class="ps-4">
                            <h5 class="text-light mb-0">Masa Kalibrasi</h5>
                            <h1 class="text-light mb-0" data-toggle="counter-up"> <?=$totalTidakValid; ?></h1>
                        </div>
                    </div>
                </div>
            </div>
...
$ambildata = mysqli_query($koneksi, "select * from md_alat");
while ($tampil = mysqli_fetch_array($ambildata)){
    ...
    if ($tampil['kalibrasi_ulang'] < $tanggal_sekarang_formatted) {
        // Menampilkan pesan toast jika kalibrasi ulang terlewat
                // Mengubah warna nama_alat menjadi berwarna merah jika tanggal kalibrasi ulang telah lewat
                $tampil['nama_alat'] = '<span style="color:red;">' . $tampil['nama_alat'] . '</span>';
        echo "
        <tr style='font-size: 14px;'>
            ...
            <td style='text-align:center;'> 
                <a href='delete.php?hapus=$tampil[id]'  style='float:right; margin-right:10px ;border-left: 1px solid black; padding-left: 15px; margin-left: 1px;' onClick = \"return confirm('Yakin akan menghapus data?');\"> hapus</a>
                <a href='detail.php?id=$tampil[id]' style='float:right; margin-right:10px ;border-left: 1px solid black; padding-left: 15px; margin-left: 1px;'> Detail</a>
                <a href='edit.php?id=$tampil[id]' style='float:right; margin-right:10px ;border-left: 1px solid black; padding-left: 15px; margin-left: 1px; padding-right:5px;'> Edit</a>
                <a href='validasi.php?id=$tampil[id]' style='float:right; margin-right:10px ; padding-left: 15px; padding-right:1px; margin-left: 5px;'> Validasi</a>
                </td>
        </tr>";
        $no++ ;
    }
